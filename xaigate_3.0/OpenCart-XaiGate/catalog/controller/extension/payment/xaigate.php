...
class ControllerExtensionPaymentXaiGate extends Controller {
	...
	{
		$this->language->load('extension/payment/xaigate');
		...
		$data_request = [
			...
			'email'		=> $email,
			'apiKey'	=> $apikey,
			'successUrl'=> $store_url . 'index.php?route=extension/payment/xaigate/response',
			'failUrl'   => $store_url . 'index.php?route=extension/payment/xaigate/response&fail=1',
			'notifyUrl' => $store_url . 'index.php?route=extension/payment/xaigate/callback',
			'description' => $desc
		];
		...
		$this->load->model('checkout/order');
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/xaigate')) {
			return $this->load->view($this->config->get('config_template') . '/template/extension/payment/xaigate', $data);
		} else {
			return $this->load->view('/extension/payment/xaigate', $data);
		}
	}
	...
	public function callback()
	{
		...
		$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_xaigate_order_status_id'), $comment, $notify = true, $override = false);
